...
function NavEquipmentList() {
  ...
  homeLink.addEventListener("click", function () {
    apiAction.getRequest(
      "https://localhost:44372/api/EquipmentList",
      (data) => {
...
function NavEquipmentSingle() {
  const equipmentNameElements = document.querySelectorAll(
    ".equipment_name, .equipment_image, .description_link"
  );
  equipmentNameElements.forEach((element) => {
    element.addEventListener("click", function () {
      const equipmentId = element.id;
      apiAction.getRequest(
        `https://localhost:44372/api/EquipmentList/${equipmentId}`,
        (data) => {
...
function AddEquipment() {
  ...
  saveEquipmentButton.addEventListener("click", function () {
    ...
    if (equipName == "") {
      document.getElementById("helpRequired").innerText =
...
function UpdateEquipmentBtn() {
  const updateEquipmentElement = document.querySelectorAll(
    ".updateEquipmentBtn"
  );
...
function CategoryDropdownNav() {
  ...
  dropdownNavElement.addEventListener("change", function () {
    ...
    apiAction.getRequest(
      `https://localhost:44372/api/Category/${dropdownValue}`,
      (category) => {
        var isAdmin = cookieAction.getCookie("userIsAdmin");
        if (isAdmin === "true") {
